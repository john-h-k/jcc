...
static bool try_de_ ## ty (MAYBE_UNUSED struct json_de_ctx *ctx, const struct json_value *value, tag ty *name) \
END_NO_UNUSED_ARGS
...
TRY_DE_STRUCT(pub_diagnostics_caps, caps) {
  *caps = (struct pub_diagnostics_caps){0};
  ...
  if (tag_support) {
    TRY_DE_ARRAY_FIELD(TYPE, tag_support, caps->tag_support.values, caps->tag_support.num_values, "valueSet", false, diagnostic_tag);
  }
  ...
  TRY_DE_BOOL_FIELD(object, caps->code_desc_support, "codeDescriptionSupport", false);
...
TRY_DE_STRUCT(window_caps, caps) {
  *caps = (struct window_caps){0};
  ...
  TRY_DE_BOOL_FIELD(object, caps->work_done_progress, "workDoneProgress", false);
...
TRY_DE_STRUCT(general_caps, caps) {
  *caps = (struct general_caps){0};
  ...
  if (stale_req) {
    TRY_DE_BOOL_FIELD(stale_req, caps->stale_req_support.cancel, "cancel", true);
    // TRY_DE_STR_FIELD(stale_req, arams->client_info.version, "version", false);
  }

  TRY_DE_ARRAY_FIELD(TYPE, object, caps->position_encodings, caps->num_position_encodings, "positionEncodings", false, pos_encoding_kind);
...
TRY_DE_STRUCT(client_caps, caps) {
  *caps = (struct client_caps){0};
  ...
  TRY_DE_TYPE_FIELD(object, text_doc_caps, caps->text_doc_caps, "textDocument", false);
...
TRY_DE_STRUCT(init_params, params) {
  *params = (struct init_params){0};
  ...
  if (client_info) {
    ...
    TRY_DE_STR_FIELD(client_info, params->client_info.version, "version", false);
  }
  ...
  TRY_DE_TYPE_FIELD(object, client_caps, params->client_capabilities, "capabilities",
                    true);
...
bool try_de_req_msg(struct json_de_ctx *ctx, const struct json_value *value, struct req_msg *msg) {
  if (!METHODS) {
...
#undef REQ_METHOD
  ...
  *msg = (struct req_msg){0};
  ...
  case REQ_MSG_METHOD_TEXTDOCUMENT_DIDOPEN:
    return try_de_didopen_textdoc_params(ctx, params, &msg->didopen_textdoc_params);
