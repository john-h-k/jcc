...
// ********** PROFILER **********
// * Regions are non re-entrant parts of code used for profiling "sections" e.g
// linking
//   - `PROFILE_BEGIN(ident)` and `PROFILE_END(ident)`
//   - `ident` must be an actual identifier token, not a string (as it uses it
//   to construct variables)
// * Multi-regions are used for profiling sections that are executed many times
// (e.g individual functions)
//   - `PROFILE_CREATE_MULTI(ident)` creates the metadata needed
//   - `PROFILE_BEGIN_MULTI(ident)` and `PROFILE_END_MULTI(ident)` are used to
//   enter and exit a multi-region
// * The macro `PROFILE_SELF` can be defined to get the profiler to show
// overhead information about the profiling itself
//   - Profiling times will _not_ include this overhead, but profiling sections
//   containing it will
// ******************************
...
#define PROFILE_CREATE_MULTI(nm)                                               \
  static struct profiler_multi_region profiler_multi_region_##nm = {           \
      .name = #nm, .entry = {0}}
#define PROFILE_BEGIN_MULTI(name)                                              \
...
NOINLINE struct profiler_multi_region_inst
