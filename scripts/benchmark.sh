...
benchmark-parse() {
  ...
  compilers="clang"
  compilers="$compilers,./build/jcc"

  mkdir -p .tmp
  _mk_sqlite3
  # _mk_huge
  sources=".tmp/huge.c"
  sources=".tmp/sqlite3/sqlite-amalgamation-3490100/sqlite3.c"

  output=".tmp/benchmark_parse.md"
  warmup=10
  min=10
  ...
  hyperfine \
    ...
    '{compiler} -fsyntax-only -O0 -g0 --no-warnings -I ../csmith-jcc/runtime {source}'
