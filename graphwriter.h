...
enum graph_strictness {
  GRAPH_STRICTNESS_STRICT,
  GRAPH_STRICTNESS_UNSTRICT
};
...
struct graphwriter *graphwriter_create(struct arena_allocator *arena, enum graph_ty ty, enum graph_strictness strictness, FILE *file);
...
struct graph_edge *edge(struct graphwriter *gwr, struct graph_vertex *from, struct graph_vertex *to);
...
void vertex_attr(struct graph_vertex *vertex, const struct graph_vertex_attr *attr);
...
FILE *begin_vertex_attr(struct graph_vertex *vertex, enum graph_vertex_attr_ty ty);
